...
class GAN(object):
    ...
    def generator(self, z, is_training=True, reuse=False):
        with tf.variable_scope("generator", reuse=reuse):
            net = tf.nn.relu(bn(linear(z, 256, scope='g_fc0'), is_training=is_training, scope='g_bn0'))
    ...
    def build_model(self):
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            self.d_optim = YFOptimizer(self.learning_rate) \
                      .minimize(self.d_loss, var_list=d_vars)
            self.g_optim = YFOptimizer(self.learning_rate*5) \
    ...
    def train(self):
        ...
        self.checkpoint_path = self.saver.last_checkpoints[-1]
    ...
    def load(self, checkpoint_dir):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            self.checkpoint_path = os.path.join(checkpoint_dir, ckpt_name)
